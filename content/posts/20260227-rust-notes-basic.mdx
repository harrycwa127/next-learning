---
title: Rust Notes (Basic)
description: Rust Notes (basic)
slug: rust-notes-basic
date: 2026-03-02
...
- **分配方式**： 自動分配與釋放。當函數執行時，變數進入堆疊；函數結束時，變數自動被「彈出」清除。

- **速度**： 極快，處理器有專門的指令來操作堆疊指標。
- **管理**： 由 CPU 緊密管理，不會產生碎片。
- **限制**： 大小固定，通常很小（通常只有幾 MB），超過會導致 Stack Overflow。
- **型別限制**： 只能存放編譯時已知大小的資料（例如：i32、常數長度陣列、原始指標）。

`堆積（Heap）`

> 「手動且靈活」的大型儲存庫

- **分配方式**： 手動或由執行時期（Runtime）分配。你需要尋找足夠大的連續空間來存放資料。
- **速度**： 較慢。需要透過系統呼叫（System Call）尋找空間，且必須透過指標（Pointer）來訪問（多了一層間接跳轉）。
- **管理**： 容易產生「記憶體碎片」。在 C/C++ 中需手動 free，而在 Java/Python 中由 GC 管理，在 Rust 中則透過「所有權系統」自動釋放。
- **限制**： 大小靈活，只要硬體記憶體夠大，幾乎沒有限制。
- **型別限制**： 用於存放動態大小或生命週期需超過函數範圍的資料（例如：String、Vec、Box）。

### Type

**Box：最簡單的智慧指標**  

`Box<T>` 是最簡單的智慧指標，它將資料分配到堆積（Heap）而不是堆疊 (Stack)上：
...
不具備所有權的觀察者，可以指向 Rc 或 Arc 管理的資料，但不會增加引用計數。

解決「循環引用」。例如：父節點擁有子節點（使用 Rc），但子節點想指回父節點時，應使用 Weak，否則這兩個物件永遠不會被釋放。
...
| Type | Storage | Ownership | Thread-Safe? |
| --- | --- | --- | --- |
| `Box` | Heap | Single | Yes |
| `Rc` | Heap | Multiple | No |
| `Arc` | Heap | Multiple | Yes |
...
| Type | Access Check | Thread-Safe? | Main Use Case |
| --- | --- | --- | --- |
| `RefCell` | Runtime | No | Mutating data inside an `Rc`. Panics if borrow rules are broken. |
| `Mutex` | Runtime | Yes | Mutating data inside an `Arc`. Blocks the thread (locks) until access is granted. |
